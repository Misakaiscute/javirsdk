    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        while(true){
            ...
            if (speed == null) {
                ...
                speed = IRSDKHeader.getInstance().getVarHeaderEntry(speed_idx);
            }
            IRSDKVarBuf curBuf = IRSDKHeader.getInstance().varBuf[IRSDKHeader.getInstance().getCurBuf()];
            float[] speedVal = curBuf.getFloats(speed.getOffset(), speed.getCount());
            for (int i = 0; i < speedVal.length; i++) {
